...
def main():
    ...
    for chr_temp in chr_list:
        ...
        seqs_cov_files = [
            os.path.join(seqs_cov_dir, 'cage_rep1.h5'),
            # IGNORE CAGE REP2 for now
        ...
        for ti in range(num_targets):
            ...
            tmp = seqs_cov_open['targets'][options.start_i:options.end_i,:]
            if ti > 0:
                if model == 'epi':
        ...
        if options.umap_bed is not None:
            ...
            mseqs_unmap = annotate_unmap(model_seqs, options.umap_bed,
            ...
            model_seqs = [model_seqs[i] for i in range(len(model_seqs)) if mseqs_map_mask[i]]
        ...
        with tf.io.TFRecordWriter(tfr_file, tf_opts) as writer:
            for si in range(num_seqs):
                ...
                hic_slice = hic_matrix[hic_start_idx-TT:hic_start_idx+TT,hic_start_idx-TT:hic_start_idx+TT]
...
#                try:
 ...
 #               except ValueError: # deal with final window being too small
  #                  continue
                ...
                Y = targets_y[hic_start_idx-TT:hic_start_idx+TT]
                X_1d = targets[hic_start_idx-TT:hic_start_idx+TT,:,1:]
